...
class MuZeroConfig:
    def __init__(self):
        ...
        self.PER_alpha = 0.35 # How much prioritization is used, 0 corresponding to the uniform case, paper suggests 1
...
class Game(AbstractGame):
    ...
    def __init__(self, seed=None):
        ...
        self.env.configure(
            {    # 需要在程序启动这个观测器之前使用自定义的公式来对观测车辆的初始速度和初始位置进行初始化
                'observation': {"type": "Kinematics",  # 使用这个观测器作为状态空间，可以获取观测车辆位置、观测车辆速度和观测车辆转向角
                                ...
                                "absolute": False,  # 使用相对坐标，相对于观测车辆。为True时使用相对于环境的全局坐标系。
                ...
                'lanes_count': 4,  # 车道数量
                "lane_change_reward": 0.8,  # 车辆换道获得的奖励
                ...
                'collision_reward': -1,  # 与车辆相撞时获取的奖励
    ...
    def step(self, action):
        ...
        # action = {
        #     "longitudinal_acceleration": numpy.tanh(action["longitudinal_acceleration"]),
        #     "lateral_acceleration": numpy.tanh(action["lateral_acceleration"]),
        #     "steering": numpy.tanh(action["steering"])
        # }
        observation, reward, done, _, _ = self.env.step(action)
        logger.info(f"reward {reward}")
        observation = observation.reshape((147,))
        logger.info(f"Observation reset step shape after step:{type(observation)}-shape-{observation.shape}")
        return numpy.array([[observation]]), reward, done, _, _
    ...
    def reset(self):
        ...
        # logger.info(f"Observation reset shape after step:{type(observation)}-shape-{observation.shape}")
        return observation
