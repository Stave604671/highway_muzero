...
class MuZeroConfig:
    def __init__(self):
        ...
        ### Game
        self.observation_shape = (1, 21, 5)  # Dimensions of the game observation, must be 3D (channel, height, width). For a 1D array, please reshape it to (1, 1, length of array)
...
class Game(AbstractGame):
    ...
    def __init__(self, seed=None, monitor_path=None, cfg_file=""):
        ...
        self.env = gym.make('highway-fast-v0', render_mode="rgb_array",config=cfg)
        # self.env.configure(
        # )
        self.env.reset()
        # if seed is not None:
    ...
    def reset(self):
        ...
        # logger.info(f"{observation.shape}")
        return observation
